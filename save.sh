...
if [ "${BACKUP_DB}" = "true" ]; then
    ...
    for engine in ${DB_ENGINE_TO_BACKUP[*]}; do
        if test -f "${ROOT}/save.d/engines/${engine}.sh"; then
...
if [ "${BACKUP_SERVICES}" = "true" ]; then
    ...
    for service in ${SERVICES_TO_BACKUP[*]}; do
        ...
        if test -f "${ROOT}/save.d/services/${service}.sh"; then
...
for backup_folder in ${FOLDERS_TO_BACKUP[*]}; do
    echo "[$($LOGDATE)] Backup ${backup_folder}"
    filter_folder=${backup_folder//\//_}
    cd ${backup_folder}
    if [ "${ONLY_SUBFOLDERS}" = "true" ]; then
        mkdir ${WORKING_DIR}/folders/${filter_folder}
        for folder in $(find ${backup_folder} -mindepth 1 -maxdepth 1 -type d); do
